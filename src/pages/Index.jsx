...
const Index = () => {
  ...
  const audio1Ref = useRef(null); // Countdown Music
  const audio2Ref = useRef(null); // Background Music
  ...
  useEffect(() => {
    if (currentComponent === 1 && audio1Ref.current) {
      ...
      audio1Ref.current
        .play()
        .catch((err) => console.log("Autoplay block (countdown):", err));
      ...
      audio2Ref.current
        .play()
        .catch((err) => console.log("Autoplay block (background):", err));
    }
  }, [currentComponent]);
          ...
          currentComponent === 0 ?(
            <div className="relative z-10 min-h-screen flex items-center justify-center">
              <motion.button
                ...
                className="px-12 py-4 text-xl font-semibold text-white bg-gradient-romantic 
                  rounded-full shadow-romantic hover:shadow-glow transform transition-all duration-300"
